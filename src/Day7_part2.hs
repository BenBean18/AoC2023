...
-- higher is better
charRanks = reverse ['A', 'K', 'Q', 'T', '9', '8', '7', '6', '5', '4', '3', '2', 'J']

charRank :: Char -> Int
charRank c = fromJust (elemIndex c charRanks) + 1

indicesOf :: (Eq a) => [a] -> a -> [Int]
indicesOf list element =
    let zipped = zip list [0..length list - 1]
        in map snd (filter (\(e, i) -> e == element) zipped)

replaceWithAll :: String -> Int -> [String]
replaceWithAll s i = map (\c -> take i s ++ [c] ++ drop (i + 1) s) charRanks
...
handRank :: String -> Int
handRank s =
    ((maximum (map handTypeRank (allHands s))) * 15 * 15 * 15 * 15 * 15 * 15 * 15) -- high enough to have more of an impact than the largest possible charRank
