...
reachableRange workflow output =
    let name = head (splitOn "{" workflow)
        ...
        conditions = map rangeForCondition conditionStrs ++ [(('x', [1 +=+ 4000]), defaultOutput)] in {-(trace $ show conditions)-} (name, (invertAll $ unreachableRange' conditions output emptyMap) `intersectAll` defaultMap)
...
unreachableRange' :: [((Char, [Range Int]), String)] -> String -> Map.Map Char [Range Int] -> Map.Map Char [Range Int]
...
-- note: here there will be only one x, m, a, s <-- FALSE FALSE FALSE
-- so this is difficult because once you hit a valid condition you stop
-- but for the example "px{a<2006:qkq,m>2090:A,rfg}"
-- you hit accepted at m>2090, so you should stop there
-- but in contrast, you can have "lnx{m>1548:A,A}"
-- accepted if m>1548, but don't want to stop there since there are other ways to get to A
unreachableRange' [] _ m = m
unreachableRange' (condition:conditions) name currentMap =
    let ((p, r), out) = condition in
        if out == name then unreachableRange' conditions name currentMap
        else unreachableRange' conditions name (Map.insert p ((currentMap Map.! p) `intersection` r) currentMap)
